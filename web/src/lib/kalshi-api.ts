 ...
 *
 * Used to re-check fill status after placing a limit order — the initial
 * response may show fill_count=0 even if the order fills moments later
 * within its 30-second expiration window.
 */
 ...
 *
 * MUST be called before resetting position to FLAT when an order got
 * 0 fills — otherwise the order can fill within its 30s expiration
 * and the system loses track of the position.
 */
